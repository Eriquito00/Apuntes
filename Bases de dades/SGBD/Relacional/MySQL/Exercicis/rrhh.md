...
LIMIT 1;
```
### 1.2.5 Consultes amb subconsultes
...
```MYSQL
SELECT e.empleat_id, e.cognoms, d.departament_id
	FROM empleats e
    INNER JOIN departaments d ON d.departament_id = e.departament_id
WHERE d.departament_id = (SELECT d.departament_id
		FROM empleats e
		INNER JOIN departaments d ON d.departament_id = e.departament_id
	WHERE CONCAT(e.nom, " ", e.cognoms) = "Pat Fay") 
    AND CONCAT(e.nom, " ", e.cognoms) != "Pat Fay";
...
```MYSQL
SELECT e.empleat_id, e.cognoms, e.salari
	FROM empleats e
    INNER JOIN departaments d ON d.departament_id = e.departament_id
...
```MYSQL
SELECT e.nom, e.cognoms, e.data_contractacio
	FROM empleats e
WHERE e.data_contractacio > (SELECT e.data_contractacio
		FROM empleats e
		INNER JOIN departaments d ON d.departament_id = e.departament_id
	WHERE CONCAT(e.nom, " ", e.cognoms) = "Pat Fay")
    AND CONCAT(e.nom, " ", e.cognoms) != "Pat Fay";
...
```MYSQL
SELECT e.cognoms, e.salari, d.departament_id
	FROM empleats e
    INNER JOIN departaments d ON e.departament_id = d.departament_id
WHERE e.salari > (SELECT AVG(e2.salari)
		FROM empleats e2
        INNER JOIN departaments d2 ON d2.departament_id = e2.departament_id
	WHERE d2.nom = "Compres")
    AND d.nom != "Compres"
ORDER BY e.salari;
```
...
```MYSQL
SELECT e.cognoms, e.salari, d.departament_id, d.nom
	FROM empleats e
    INNER JOIN departaments d ON e.departament_id = d.departament_id
WHERE e.salari > (SELECT AVG(e2.salari)
		FROM empleats e2
...
```MYSQL
SELECT e.empleat_id, e.nom, e.cognoms
	FROM empleats e
WHERE e.empleat_id = (SELECT e2.id_cap
		FROM empleats e2
	WHERE e2.empleat_id IN(103));
```
...
SELECT d.departament_id, d.nom
	FROM departaments d
WHERE d.departament_id NOT IN (SELECT DISTINCT e.departament_id
		FROM empleats e
...
```MYSQL
SELECT CONCAT(LEFT(e.nom,1),".",e.cognoms), e.salari, d.nom
	FROM empleats e
    INNER JOIN departaments d ON e.departament_id = d.departament_id
WHERE e.salari > (SELECT AVG(e2.salari)
		FROM empleats e2
        INNER JOIN departaments d2 ON e2.departament_id = d2.departament_id
	WHERE d.departament_id = d2.departament_id
	GROUP BY d2.departament_id);
```
...
WHERE e.empleat_id = ANY (SELECT hf.empleat_id
		FROM historial_feines hf
	GROUP BY hf.empleat_id
    HAVING COUNT(hf.empleat_id) = 1)
