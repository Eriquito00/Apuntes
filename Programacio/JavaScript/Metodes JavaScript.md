...
![](/Imatges/Pasted%20image%2020240916191842.png)
...
- trim(): retorna una cadena eliminant els espais del principi i del final. També hi ha `trimStart()` i `trimEnd()` que només eliminen els del principi o els del final, respectivament.
...
- padStart(n, text) i padEnd(n, text): enganxa `text` a l'_string_ tantes vegades com sigui necessari fins que tingui la longitud indicada en `n`.  
...
- splice(posicio, nº eliminacions): Permet afegir o eliminar elements a una array.
...
JavaScript té aquesta classe per ajudar en la realització de càlculs matemàtics.  
